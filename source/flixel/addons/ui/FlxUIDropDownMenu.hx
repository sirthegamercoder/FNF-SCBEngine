...
class FlxUIDropDownMenu extends FlxUIGroup implements IFlxUIWidget implements IFlxUIClickable implements IHasParams
{
			...
			?DropPanel:FlxUI9SliceSprite, ?ButtonList:Array<FlxUIButton>, ?UIControlCallback:Bool->FlxUIDropDownMenu->Void)
	{
		...
		if (dropPanel == null)
		{
			...
			// Set dropPanel background to white (Psych style)
			dropPanel.color = FlxColor.WHITE;
		}
	...
	private function makeListButton(i:Int, Label:String, Name:String):FlxUIButton
	{
		...
		var normalGraphic = FlxUIAssets.IMG_INVIS;
		var hoverGraphic = FlxUIAssets.IMG_HILIGHT;
		
		t.loadGraphicSlice9([normalGraphic, hoverGraphic, hoverGraphic], Std.int(header.background.width),
		...
		t.onOver.callback = function() {
			t.color = 0xFF0066FF;
		};

		t.down_color = FlxColor.WHITE;
		t.onOut.callback = function() {
			t.color = FlxColor.WHITE;
		};
